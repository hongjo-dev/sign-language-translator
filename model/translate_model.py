...
file_path = 'sign-language-translator/model/수어_한국어.xlsx'
df = pd.read_excel(file_path)

src_data = df['의미'].tolist()
trg_data = df['수어'].tolist()
...
# 단어장을 파일로 저장
with open('model/src_vocab.json', 'w', encoding='utf-8') as f:
    json.dump(src_vocab, f, ensure_ascii=False, indent=4)

with open('model/trg_vocab.json', 'w', encoding='utf-8') as f:
...
class Encoder(nn.Module):
    ...
    def forward(self, src, src_mask):
        ...
        # 모든 인코더 레이어를 차례대로 거치면서 순전파(forward) 수행p
...
if __name__ == "__main__":
    ...
    src_vocab = input_data['src_vocab']
    trg_vocab = input_data['trg_vocab']

    # 전체 모델 객체 불러오기
    model = torch.load("C:/졸작_범_test/train/ko_zzz.pth", map_location=torch.device('cuda'))

    translation, _ = translate_sentence(sentence, src_vocab, trg_vocab, model, 'cuda')
